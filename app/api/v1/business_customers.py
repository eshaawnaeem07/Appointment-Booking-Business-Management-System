...
def get_customers(id: str, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
...
def get_customer(id: str, customer_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
...
def create_customer(id: str, payload: CustomerCreate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
...
def update_customer(id: str, customer_id: int, payload: CustomerUpdate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
...
def delete_customer(id: str, customer_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
